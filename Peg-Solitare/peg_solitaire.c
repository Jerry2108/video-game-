...
int main(int argc, char *argv[]) {
	...
	if( ai_run ){ 
		...
		printf("SOLUTION:                               \n");
		print_solution( );
		fprintf(outputFile, "STATS: \n");
		fprintf(outputFile, "\tExpanded nodes: %'d\n\tGenerated nodes: %'d\n", expanded_nodes, generated_nodes);
		fprintf(outputFile, "\tSolution Length: %d\n", solution_size);
		fprintf(outputFile, "\tNumber of Pegs Left: %d\n", num_pegs( &(solution[solution_size]) ) );
		fprintf(outputFile, "\tExpanded/seconds: %d\n", (int)(expanded_nodes/cpu_time_used) );
		fprintf(outputFile, "\tTime (seconds): %f\n", cpu_time_used );
		fclose(outputFile); 
